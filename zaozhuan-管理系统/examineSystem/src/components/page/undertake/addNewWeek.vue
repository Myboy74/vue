    ...
    <div class="top">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>周报添加</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="f_top">
        ...
        methods: {
            ...
            onBtn(){  //提交
                ...
                this.addArray.forEach(item => {
                    this.data.weeks.push({  //保存本周周报全部内容
                        keyPoint:item.name.replace(/(^\s*)|(\s*$)/g, ""),
                        responsiblePerson:item.keyWork.replace(/(^\s*)|(\s*$)/g, ""),
                        weekResult:item.desc.replace(/(^\s*)|(\s*$)/g, "")
                    })
                });
                this.addWork.forEach(item => {
                    this.data.nextWeekPlans.push({//保存下周计划全部内容
                        keyPoint:item.name.replace(/(^\s*)|(\s*$)/g, ""),
                        responsiblePerson:item.keyWork.replace(/(^\s*)|(\s*$)/g, ""),
                        weekResult:item.desc.replace(/(^\s*)|(\s*$)/g, "")
                    })
                ...
                postAjax("week/reportWeek", this.data).then(res => {
