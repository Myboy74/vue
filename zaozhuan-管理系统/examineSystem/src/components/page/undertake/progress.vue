    ...
    <div class="table">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>上报进度</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        ...
        <div class="handle-box">
          ...
          <el-table
            ...
            prop="newProgress"
            label="最新进展">
            <span slot-scope="scope" :title="scope.row.progressContent">{{ scope.row.progressContent?scope.row.progressContent:' ' }}</span>
            </el-table-column>
            <el-table-column
            prop="overallPlan"
            label="整体进度">
            <span slot-scope="scope" :title="scope.row.completionPercentage">{{ scope.row.completionPercentage?scope.row.completionPercentage+`%`:' ' }}</span>
            </el-table-column>
            <el-table-column
            prop="taskEndTime"
            label="完成期限">
            ...
            prop="taskStatus"
            label="操作" >
            <a slot-scope="scope" @click="progressInfo(scope.row)"   style="color:#409eff;cursor: pointer;">{{   getStatus(scope.row.planState) }}</a>
        ...
        if (res.data.code == 200) {
          ...
          if (res.data.data.length) {
            ...
            searchInfor(){
                ...
                postAjax("taskPlan/queryReportProgressList",datas).then(res => {
                    if (res.data.code == 200) {
            ...
            getassign() {
                ...
                postAjax("taskPlan/queryReportProgressList",data).then(res => {
                    if (res.data.code == 200) {
                        this.total=res.data.data.total
                      this.tableData= res.data.data.dataList ;
