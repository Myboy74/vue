                  ...
                  <el-input
                    ...
                    v-model="filterText">
                      ...
                      <el-tree
                        ...
                        highlight-current
                        :load="loadNode"
                        :default-checked-keys="this.checkedIdString"
                        lazy
...
export default {
  ...
  methods: {
      ...
      //获取任务详情
          getDetailsTask() {
      getA("oa_task/query_task_id", {
        id: this.id

      }).then(res => {
        if (res.data.code == 200) {
            let item = res.data.data;
            item.isReportPlan == 1 ? (this.form.delivery = true) : (this.form.delivery = false);
            item.openTaskDeadline == 1? (this.form.enddelivery = true) : (this.form.enddelivery = false);
            this.form.name = item.taskTitle
            this.form.typeListChildId = item.taskTypeId
            this.form.desc = item.taskTarget
            this.form.startTime = item.reportTime
            this.form.endTime = item.missionDeadline
            this.form.assess = item.evaluationMode
            this.form.descs = item.remark
            this.checkedId = item.organizations.split(',')
            this.checkedIdString = item.organizations.split(',')
            var _this = this;
            item.attachments.forEach(function(item){
            _this.form.fileList.push({name:item.attachmentName,url:item.attachmentUrl});
          })
          item.organizationVOS.forEach(function(item){
            _this.checkArray.push({id:item.id,label:item.label});
          })
        // this.checkArray=this.newArrayChecked;
    ...
    addTask() {
      if (this.form.delivery == true) {
        ...
        this.form.enddelivery == true
          ...
          taskPlanState: "01"});
        postA("oa_task/update_task",dataObj)
        .then(res => {
          if (res.data.code == 200) {
        ...
        this.form.enddelivery == true
          ...
          remark: this.form.descs,
          taskPlanState: "04"});
        postA("oa_task/update_task", dataObjs).then(res => {
          if (res.data.code == 200) {
            this.$message({ type: "success", message: "下发成功!" });
             this.$router.push({ path:'/taskManage',});
          }else{
    ...
    saveTask() {
      ...
      postA("oa_task/update_task",data).then(res => {
        if (res.data.code == 200) {
