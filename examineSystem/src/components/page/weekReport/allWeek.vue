    ...
    <div class="f_tab">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>本周周报查看</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
    ...
    export default {
        ...
        methods:{
            ...
            getWeekAll(val) {
                ...
                postAjax("week/queryCurrentWeekAudited",data1).then(res => {
                    if (res.data.code == 200) {
                        if(res.data.data.length==0){
                            this.lock=true;  //提示没有数据
                            this.completed=[];  //先清空，只要搜索到的
                            return;
                        }else{
                            this.lock=false; //隐藏没有数据的提示
                            this.completed=[];  //先清空，只要搜索到的
            ...
            getWorkCompleted(val){  //本周已完成工作
                let data1=qs.stringify(val);
                postAjax("week/queryCurrentFinishWeekAudited",data1).then(res => {
                    if (res.data.code == 200) {
                        if(res.data.data.length==0){
                            this.lockWord=true;   //提示没有数据
                            this.workComplete=[];  //没有清空原先的数据
                            return;
                        }
                        this.lockWord=false; //隐藏没有数据的提示
            ...
            getWorkNoCompleted(val){//本周未完成工作
                let data1=qs.stringify(val);
                postAjax("week/queryCurrentNotFinishWeekAudited",data1).then(res => {
                    if (res.data.code == 200) {
                        if(res.data.data.length==0){
                            this.lockNoWord=true;   //提示没有数据
                            this.workNoComplete=[];  //先清空，只要搜索到的
                            return;
                        }
                        this.lockNoWord=false; //隐藏没有数据的提示
                        this.workNoComplete=[];   //先清空，只要搜索到的
            ...
            getPlan(val){
                ...
                postAjax("nextWeekPlan/queryNextWeekPlan",data1).then(res => {
                    if (res.data.code == 200) {
                        if(res.data.data.length==0){
                            this.lockPlan=true;   //提示没有数据
                            this.plan=[];  //先清空，只要搜索到的
                            return;
                        }
                        this.lockPlan=false;    //隐藏没有数据的提示
                        this.plan=[];  //先清空，只要搜索到的
