    ...
    <div class="table">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item style="text-align:left">上报进度</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        ...
        <div class="periodBox" >
            ...
            <div class="clearfix" style="margin-bottom:10px">
                ...
                <label class="label-search" style="width:150px;">计划周期:</label>
                   ...
                   <el-select v-model="value" placeholder="请选择" @change="changePeriod">
                        <el-option
                        v-for="(item,index) in periodArray"
                        :key="index+1"
                        :label="item.label"
                        :value="item.value">
                        </el-option>
        ...
        <div style="margin-bottom:10px">
            ...
            <p class="p_style">{{completionCriteria}}</p>
        </div>
        <div style="margin-bottom:10px">
            <label class="label-search" style="width:150px;">周期内容： </label>
            <p class="p_style">{{periodContent}}</p>
        </div>
        <hr style="border:none;border-bottom:1px solid #ccc;margin:20px 0;">
        <div v-for="(item,index) in progressInfo" :key="index+1"  v-show="progressInfo.length!=0">
            <div style="margin-bottom:10px">
        ...
        <div style="margin-bottom:10px" class="clearfix">
            <label class="label-search" style="width:150px;">任务完成百分比： </label>
            <el-input v-model="percent"  onkeyup="this.value=this.value.replace(/\D/g,'')"  onafterpaste="this.value=this.value.replace(/\D/g,'')" maxlength="3" size="14" placeholder="请输入1-100的数字" style="width:20%;margin-left:15px"></el-input>
        </div>

        <div>
            <label class="label-search" style="width:150px;">最新进展情况： </label>
            <el-input
            type="textarea"
            :rows="6"
            placeholder="请输入内容"
        ...
        <el-button type="primary" @click="sendProgress">提交进度</el-button>
...
import qs from "qs";
    export default {
        ...
        methods: {
            ...
            sendProgress(){
                if(this.percent>100){
                    this.$message.error("请输入小于100得整数");
                    return
                }
            let content={periodId:this.value,completionPercentage:this.percent,progressContent:this.newProgress,planId:this.planId}
                 postAjax("progress/saveProgress",content).then(res => {
                    if (res.data.code == 200) {
